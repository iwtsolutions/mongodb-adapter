...
function MongoDB(s, schema, callback) {
    ...
    var server;
    if (s.rs) {
        if (!s.server.hasOwnProperty('auto_reconnect')) {
            s.server.auto_reconnect = true;
        }
        set = [];
        for (i = 0, n = s.hosts.length; i < n; i++) {
            set.push(new mongodb.Server(s.hosts[i], s.ports[i], s.server));
        }
        server = new mongodb.ReplSetServers(set, {rs_name: s.rs});

    } else {
        server = new mongodb.Server(s.host, s.port, s.server);
    }
